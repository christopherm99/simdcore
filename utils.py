...
def read_float16_from_binary(filename, count=8): return np.frombuffer(open(filename, 'rb').read(), dtype=np.half)
def save_memory_to_binary(filename, memory, count=16): return np.frombuffer(memory[:count*2], dtype=np.half).tofile(filename) or as_np(memory[:count*2])
